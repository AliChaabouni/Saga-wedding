...
export function setupDevisHandlers() {
  ...
  ipcMain.handle('add-devis', async (event, devisData) => {
    const { client_id, date_creation, date_validite, statut, total_ht, tva, total_ttc, notes, items } = devisData;
    
    // Generate numero (e.g. 2026/0001)
    const year = new Date(date_creation).getFullYear() || new Date().getFullYear();
    const numero = await new Promise((resolve) => {
      db.get(`SELECT numero FROM devis WHERE numero LIKE ? ORDER BY id DESC LIMIT 1`, [`${year}/%`], (err, row) => {
        if (row && row.numero) {
  ...
  ipcMain.handle('update-devis', (event, devisData) => {
    const { id, statut, total_ht, tva, total_ttc, notes, items } = devisData;
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        db.run('BEGIN TRANSACTION');
        db.run(`UPDATE devis SET statut = ?, total_ht = ?, tva = ?, total_ttc = ?, notes = ? WHERE id = ?`,
